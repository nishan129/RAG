...
async def _ping_postgres() -> bool:
    try:
        ...
        logger.debug("Postgres health check failed: {}", e)
        return False
...
async def _ping_qdrant() -> bool:
    try:
        ...
        logger.debug("Qdrant health check failed: {}",e)
        return False
...
async def _ping_openai() -> bool:
    try:
        ...
        logger.debug("OpenAI health check Failed: {}",e)
        return False
...
async def health_check() -> dict[str, Any]:
    """
    Plug every dependency and report status,
